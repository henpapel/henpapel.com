class Almeja extends Cajas{
    ...
    appndPapelCarton(arrPrincipal, arrPapel, parte, tipo ){
        ...
        if( tipo == 'cot' ) isCot();
        if( tipo == 'carton' ) isCartones();
        if( tipo == 'mod' ) isModPapers();
    ...
    checkDatosI(grabar, modificar){
        ...
        this._bancos.find(function(banco){
            ...
            banco['ambos_lados'] = $('#chkALados'+id).prop('checked')
            banco['lleva_papel'] = $('#optPapel'+id).val()
            ...
            ambosLados  = banco['ambos_lados']
            llevaPapel  = banco['lleva_papel']

            if( llevaPapel !== 'Si') banco['papel'] = 0
            ...
            //checkDatos(ambosLados, id, 'chkALados');

            let error = false;
            if( llevaPapel == 'Si' ){
                
                error = checkDatos(papel,id,'optBan')
                banco['papel'] = parseInt(banco['papel'])
            }
        ...
        try{
            ...
            // (RESUMEN)
                ...
                this._secciones.forEach( function(sec) {
                    ...
                    this.appndPapelCarton( respuesta, respuesta[papel], sec['titulo'],'cot' );
                }.bind(this));
            ...
            // CARTONES
                ...
                this.appndPapelCarton( respuesta, respuesta['CartonCar'], "Cartón Tapa",'carton' );
                this.appndPapelCarton( respuesta, respuesta['CartonCaj'], "Cartón Cajón",'carton' );
	...
	printCotizacion(AGlobal){
	    ...
	    this._secciones.forEach( function(sec){
            ...
            this.appndPapelCarton( AGlobal, AGlobal[texto], sec['titulo'],'mod' );
	    }.bind(this));

        // CARTONES
        this.appndPapelCarton( AGlobal, AGlobal['CartonCaj'], "Cartón Cajón",'carton' );
        this.appndPapelCarton( AGlobal, AGlobal['CartonCar'], "Cartón Tapa",'carton' );
